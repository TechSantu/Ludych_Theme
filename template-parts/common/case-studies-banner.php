...
$cs_banner_bg = get_field( 'case_studies_banner_background', $post_id );
if ( ! $cs_banner_bg ) {
	$cs_banner_bg = array(
...
$cs_heading = get_field( 'case_studies_banner_heading', $post_id );
if ( ! $cs_heading ) {
	$cs_heading = 'Case Studies';
}

$cs_subtitle = get_field( 'case_studies_banner_subtitle', $post_id );
if ( ! $cs_subtitle ) {
	$cs_subtitle = 'Case Studies';
}

$cs_title = get_field( 'case_studies_banner_title', $post_id );
if ( ! $cs_title ) {
	$cs_title = 'Thoughts <span>That Shape</span> the <span>Digital Future</span>';
}
?>

<section class="inner-banner-wrap with-overlay text-center" style="background-image: url('<?php echo esc_url( $cs_banner_bg['url'] ); ?>');">
