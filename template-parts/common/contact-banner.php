...
$contact_banner_bg = get_field( 'contact_banner_background', $post_id );
if ( ! $contact_banner_bg ) {
	$contact_banner_bg = array(
		'url' => get_template_directory_uri() . '/assets/images/services-bg.jpg',
		'alt' => 'Contact us',
	);
}

$contact_banner_heading = get_field( 'contact_banner_heading', $post_id );
if ( ! $contact_banner_heading ) {
	$contact_banner_heading = 'Contact Us';
}

$contact_banner_subtitle = get_field( 'contact_banner_subtitle', $post_id );
if ( ! $contact_banner_subtitle ) {
	$contact_banner_subtitle = 'Ready to Transform Your Digital Presence?';
}

$contact_banner_title = get_field( 'contact_banner_title', $post_id );
if ( ! $contact_banner_title ) {
	$contact_banner_title = 'Let\'s Build Something<br><span>Amazing Together</span>';
}

$contact_banner_description = get_field( 'contact_banner_description', $post_id );
if ( ! $contact_banner_description ) {
	$contact_banner_description = 'Ready to transform your digital presence? Our team of experts is here to help you achieve your business goals with innovative solutions.';
}
?>

<section class="inner-banner-wrap with-overlay text-center" style="background-image: url('<?php echo esc_url( $contact_banner_bg['url'] ); ?>');">
		...
		<div class="inner-banner">
			<div class="global-header services-head">
				...
				<div class="min-title">
					...
					<h6><?php echo esc_html( $contact_banner_subtitle ); ?></h6>
				</div>
				<h5><?php echo wp_kses_post( $contact_banner_title ); ?></h5>
				<p><?php echo esc_html( $contact_banner_description ); ?></p>
			</div>
		</div>
	</div>
</section>
